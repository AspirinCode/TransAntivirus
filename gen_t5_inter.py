...
def generate(model,iupac2smile_model, tokenizer,smile_tokenizer, inputs_list, masked_indices,smiles_ids, n_candidates=1):
    ...
    for minibatch in minibatches:
        ...
        #outputs = model.generate(minibatch[:,:-1],do_sample=True,pad_token_id=pad,decoder_start_token_id=minibatch[:,1],num_return_sequences=n_candidates)
        outputs = model.generate(minibatch,do_sample=True,num_return_sequences=n_candidates)
        print(minibatch,minibatch.shape,outputs,outputs.shape)
        exit()

        smile_batch = iupac2smile_model.predict(minibatch.cpu())
...
def main():
    ...
    '''
    tokenizer = torch.load(pt.join("./","inter_iupac_tokenizer.pt"), map_location="cpu")
    smile_tokenizer = torch.load(pt.join("./","inter_smile_tokenizer.pt"), map_location="cpu")
...
#####################################################################################################
    ...
    model_fn = os.path.join('./', "9_iupac2iupac_model_inter.pt")
...
#####################################################################################################
    ...
    # load weights from checkpoint
    model_fn = os.path.join('./', "9_iupac2smile_model_inter.pt")
    state_dict = torch.load(model_fn, map_location="cpu")
    ...
    aaa.to_csv("df_final_inter.csv",sep="|",index= None)
    print(aaa)
...
#python gen_t5_inter.py  --dataset_dir ./download_pubchem/    --vocab_fn ./vocab/iupac_spm.model --dataset_filename ./pubchem_30m_new.csv